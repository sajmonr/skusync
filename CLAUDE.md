...
file and start an empty project. Starts the `web-api` HTTP host (http://localhost:5257), the Angular dashboard (http://localhost:4200), and the Shopify app — the `shopify-tunnel` ngrok endpoint fronting `web-api` at https://shopify-skusync.ngrok.app plus the `shopify-app-dev` CLI dev session — all configured in `process-compose.yaml`. Node dependencies come from one-shot `dashboard-install` / `shopify-app-install` processes, so no manual `npm install` is needed; the Shopify CLI does need a terminal for its first login and store selection. The dev connection strings in `appsettings.Development.json` already point at the `compose.yaml` services. Each host's Shopify / SkuLabs / AWS config comes from its .NET user secrets, which `dotnet run` loads automatically in Development. It also starts the `app-server` background worker (Shopify webhook handlers, SQS poller, Hangfire recurring jobs) — so the scheduled reconcile/dispatch pipeline runs end-to-end.
