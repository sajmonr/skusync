...
public class SkulabsTitleSyncTaskTests
{
    private readonly ISkulabsTitleSyncService _syncService = Substitute.For<ISkulabsTitleSyncService>();

    [Fact]
    public async Task Execute_ShouldCallSyncAll()
    {
        _syncService.SyncAll(Arg.Any<CancellationToken>()).Returns(SkulabsTitleSyncResult.Empty);
    ...
    {
        CreateSut().Name.ShouldBe(nameof(SkulabsTitleSyncTask));
    }

    private SkulabsTitleSyncTask CreateSut() =>
        new(_syncService, NullLogger<SkulabsTitleSyncTask>.Instance);
}
