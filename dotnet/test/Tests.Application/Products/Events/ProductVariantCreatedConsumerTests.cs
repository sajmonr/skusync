...
public class ProductVariantCreatedConsumerTests : IDisposable
{
    ...
    private readonly TestLogger<ProductVariantCreatedConsumer> _logger = new();

    public ProductVariantCreatedConsumerTests()
    {
    ...
    public async Task OnHandle_ShouldLogWarningAndSkipShopifyUpdate_WhenVariantNotFound()
    {
        ...
        await CreateSut().OnHandle(
            new ProductVariantCreatedEvent(Guid.NewGuid()),
            CancellationToken.None);
    ...
    private ProductVariantCreatedConsumer CreateSut() =>
